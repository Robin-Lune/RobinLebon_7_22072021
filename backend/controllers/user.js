...
exports.getUserPosts = (req, res, next) => {
  const userId = req.params.id;
  const sql = `SELECT post.id, message ,datecreation,imageurl, COUNT(DISTINCT like_post.id) AS total_like, comments.commentaire, comments.utilisateur_id AS comm_uid, comments.datecreation_comm, comments.id AS comm_id, COUNT(comments.id) AS total_comm , comm_user.nom AS comm_nom, comm_user.prenom AS comm_prenom, comm_user.imageProfile AS comm_picture  FROM post LEFT JOIN like_post ON (post.id = lp_post_id)  LEFT JOIN comments ON (comments.post_id = post.id) LEFT JOIN comments AS com ON com.id = (SELECT com.id FROM comments AS com WHERE com.post_id = post.id ORDER BY com.datecreation_comm DESC LIMIT 1) LEFT JOIN utilisateur AS comm_user ON( comm_user.UID = comments.utilisateur_id) WHERE post.utilisateur_id = '${userId}' GROUP BY post.id  ORDER BY datecreation DESC ;`;
  let query = db.query(sql, (err, result) => {
    if (err) throw err;
    res.status(200).json(result);
  });
